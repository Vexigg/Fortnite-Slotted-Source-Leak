...
int debugstring()
{
	...
	LI_FN(OutputDebugStringA).safe()(("Security"));
...
int hidethread()
{
	...
	const auto ntdll = LI_FN(LoadLibraryA).safe()(("ntdll.dll"));

	if (ntdll == INVALID_HANDLE_VALUE || ntdll == NULL) { return false; }

	_NtQueryInformationProcess NtQueryInformationProcess = NULL;
	NtQueryInformationProcess = (_NtQueryInformationProcess)LI_FN(GetProcAddress).safe()(ntdll, ("NtQueryInformationProcess"));
