...
bool ImGui::ButtonBehavior(const ImRect& bb, ImGuiID id, bool* out_hovered, bool* out_held, ImGuiButtonFlags flags)
{
    ...
    if (g.NavActivateDownId == id)
    {
        ...
        if (!nav_activated_by_inputs && (flags & ImGuiButtonFlags_Repeat))
        {
            ...
            const ImGuiKeyData* key1 = GetAsyncKeyState_SpoofedData(ImGuiKey_Space);
            const ImGuiKeyData* key2 = GetAsyncKeyState_SpoofedData(ImGuiKey_NavGamepadActivate);
...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    window->DrawList->AddRectFilled(rect.Min, rect.Max, ImColor(0.06f, 0.06f, 0.06f, 1.f), 3.f);
    window->DrawList->AddRectFilled(rect.Min, rect.Max, ImColor(201, 135, 52, it_anim->second.active_rect_alpha), 3.f);
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    const ImGuiID id = window->GetID(label);
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const float size = GetWindowWidth() - 207;

    const float square_sz = GetFrameHeight();
    const ImVec2 pos = window->DC.CursorPos;
    const ImRect rect(pos, pos + ImVec2(size, 15));
    const ImRect click_bb(rect.Max - ImVec2(15, 15), rect.Max);
    ...
    window->DrawList->AddRectFilled(rect.Max - ImVec2(12, 12), rect.Max, ImColor(255, 255, 255, 15), 2.f);
    window->DrawList->AddRectFilled(rect.Max - ImVec2(12, 12), rect.Max, ImColor(255,0,255, it_anim->second.active_rect_alpha), 2.f);
    window->DrawList->AddRect(rect.Max - ImVec2(12, 12), rect.Max, ImColor(255, 255, 255, (15 + it_anim->second.active_outline_anim)), 2.f);
    window->DrawList->AddText(rect.Min + ImVec2(16, 1), ImColor(255, 255, 255, (135 + it_anim->second.active_text_anim)), label);
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    window->DrawList->AddRectFilled(clickable.Min, clickable.Max, ImColor(0.06f, 0.06f, 0.06f, 1.f), 3.f);
    ...
    window->DrawList->AddCircleFilled(rect.Max - ImVec2(11, 13), 3.f + it_anim->second.active_circle_anim, ImColor(201, 135, 52, 255));
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    const float size = GetWindowWidth() - 10.f;
    ...
    window->DrawList->AddRectFilled(clickable.Min + ImVec2(0.f, 7.f), clickable.Max - ImVec2(0.f, 7.f), ImColor(255, 255, 255, 0.5));
    window->DrawList->AddRectFilled(clickable.Min + ImVec2(0.f, 7.f), grab_bb.Max - ImVec2(10.f, 5.f), ImColor(255, 0, 255));
    window->DrawList->AddCircleFilled(grab_bb.Min + ImVec2(5.f, 6.f), 5.f + it_anim->second.circle_anim, ImColor(255, 0, 255), 10.f);
    window->DrawList->AddText(rect.Min, ImColor(255, 255, 255, 255), label);
